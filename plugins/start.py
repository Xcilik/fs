...
from database.mongo_db import add_user, get_user, del_user
...
async def start_command(client: Bot, message: Message):
    ...
    try:
        await add_user(client.me.id, id)
    except:
        pass
    text = message.text
    if len(text) > 7:
...
async def not_joined(client: Bot, message: Message):
    ...
    try:
        await add_user(client.me.id, message.from_user.id)
    except:
        pass    
...
async def get_users(client: Bot, message: Message):
    ...
    users = await get_user(client.me.id)
...
async def send_text(client: Bot, message: Message):
    if message.reply_to_message:
        query = await get_user(client.me.id)
        ...
        for row in query:
            ...
            if chat_id not in ADMINS:
                try:
                    ...
                    await del_user(client.me.id, chat_id)
                    blocked += 1
